...
|        union         |              声明联合数据类型（详细讲解见下方）              | C/C++ |
|         enum         |                声明枚举类型（详细讲解见下方）                | C/C++ |
...
|       typedef        |   用以给数据类型取别名(当然还有其他作用)（详细讲解见下方）   | C/C++ |
...
|         new          |                 new（新建）用于新建一个对象                  |  C++  |
|        delete        |           delete（删除）释放程序动态申请的内存空间           |  C++  |
|    **const_cast**    |                   const、volatile类型转换                    |  C++  |
|   **dynamic_cast**   |                           动态转换                           |  C++  |
| **reinterpret_cast** |                        非相关类型转换                        |  C++  |
|   **static_cast**    |                         相关类型转换                         |  C++  |
|        export        | 对模板类型，在定义这些模板类对象和模板函数时，使用标准 C++ 新增加的关键字 export（导出） |  C++  |
|     **explicit**     | "禁止单参数构造函数"被用于自动型别转换，其中比较典型的例子是容器类型 |  C++  |
|        friend        | friend（友元）声明友元关系，友元可以访问与其有 friend 关系的类中的 private/protected 成员，提高了效率 |  C++  |
|        inline        | inline（内联）函数的定义将在编译时在调用处展开，inline 函数一般由短小的语句组成，可以提高程序效率 |  C++  |
...
|       operator       |               operator（操作符）用于操作符重载               |  C++  |
|     **template**     |            template（模板），C++ 中泛型机制的实现            |  C++  |
|     **decltype**     |                        推断变量的类型                        |  C++  |
|        throw         | throw（抛出）用于实现 C++ 的异常处理机制，可以"抛出"一个异常 |  C++  |
...
|       virtual        |           virtual（虚的），C++ 中用来实现多态机制            |  C++  |
...
> 问题：结合数据在内存的存储方式，当我们在栈上申请一块儿空间，作为结构体变量的存储空间，如何申请和开辟，按位怎么存放？堆上呢？
